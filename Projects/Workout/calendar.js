...
function includeCalendarEventListeners(calendarWrapper) {
    ...
    function removeIfContains(date){
      var length = selectedDatesArr.length;
      ...
      for (var i = 0; i < length; i++){
        ...
        var day = selectedDatesArr[i].day;
        var month = selectedDatesArr[i].month;
        var year = selectedDatesArr[i].year;

        if (date_day == day && date_month == month && date_year == year){
          selectedDatesArr.splice(i, 1);
          return true;
        }
    ...
    rdau = removeDateAndUpdate;

    function removeDateAndUpdate(index){
      var date = selectedDatesArr[index];
      deselectDate(date.day, date.month, date.year);
      updateDateList();
    }
    ...
    function updateDateList(){
      ...
      for (var i = 0; i < length; i++){
        ...
        innerContent += "<p style='display: inline-block; background-color: black; color: white; width: 25%; padding: 2.5%; text-align: center;'>Date: ";
        innerContent += month;
        innerContent += "/";
        innerContent += day;
        innerContent += "/";
        innerContent += year;
        innerContent += "</p>";
        innerContent += "<p style='margin-left: 15px; display: inline-block;'>Exerceise Name:</p>";
        innerContent += "<input style='display: inline-block; margin-left: 20px; width: 35%' type='text' name='name'></input>";
        innerContent += "<button onclick='rdau("
        innerContent += i
        innerContent += ")' style='background-color: black; color: white; margin-left: 20px; width: 15%'>Add to Calendar</button>"
        innerContent += "<br>";
      }

      document.getElementById("dateList").innerHTML = innerContent;
    }
